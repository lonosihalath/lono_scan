...
public class ScanView: UIView,AVCaptureMetadataOutputObjectsDelegate,FlutterPlugin {
  ...
  init(_ frame:CGRect, viewId:Int64, args: Any?,registrar: FlutterPluginRegistrar) {
    ...
    self.queue = DispatchQueue.init(label: "com.lono.scan", attributes: .concurrent);
    self.session = AVCaptureSession();
    self.channel = FlutterMethodChannel(name: "lono/scan/method_\(viewId)", binaryMessenger: registrar.messenger());
...
//      }
  ...
  public func dispose() {
    ...
    self.scanShapeLayer?.removeAllAnimations()
    self.scanShapeLayer?.removeFromSuperlayer()
    self.scanShapeLayer = nil
